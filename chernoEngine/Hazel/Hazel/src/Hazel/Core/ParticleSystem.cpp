...
namespace Hazel
{
	...
	void ParticleSystem::OnUpdate(Timestep ts) //更新需要渲染的粒子
	{
	...
	void ParticleSystem::OnRender(OrthographicCamera camera) //绘制pool存活的粒子
	{
		...
		for (auto& particle : m_ParticlePool)
		{
			...
			Renderer2D::DrawRotatedQuad(glm::vec3(particle.Position, 0.2f), { size,size }, particle.Rotation, color);
